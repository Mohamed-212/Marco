...
function addInputField_assembly(divName, assembly_info) {

    if (count == limits) {
        alert("You have reached the limit of adding " + count + " inputs");
    } else {
        if (assembly_info.length > 0) {
            assembly_info.forEach(function (element, index, array) {
                var newdiv = document.createElement("tr");
                var tabin = "product_name_" + count;
                ...
                newdiv.innerHTML =
                        ...
                        '" required="" id="product_name_' +
                        count +
                        '" ><input value='+element.child_product_id+' type="hidden" class="autocomplete_hidden_value product_id_' +
                        count +
                        '" name="product_id[]"/><input type="hidden" class="sl" value="' +
                        count +
                        '"><input type="hidden" class="baseUrl" value="' +
